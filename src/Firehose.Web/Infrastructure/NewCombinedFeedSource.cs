	...
	public class NewCombinedFeedSource
    {
		...
		private void EnsureHttpClient()
        {
            if (httpClient == null)
            {
                ...
                httpClient.DefaultRequestHeaders.UserAgent.Add(
                    new ProductInfoHeaderValue("PlanetPowerShell", $"{GetType().Assembly.GetName().Version}"));
            ...
            IEnumerable<IAmACommunityMember> tamarins, int? numberOfItems)
        {
			...
			var feed = new SyndicationFeed(
                ...
                new Uri(ConfigurationManager.AppSettings["BaseUrl"] ?? "https://planetpowershell.com"),
                numberOfItems.HasValue ? orderedItems.Take(numberOfItems.Value) : orderedItems)
            {
                ImageUrl = new Uri(ConfigurationManager.AppSettings["RssFeedImageUrl"] ?? "https://planetpowershell.com/Content/Logo.png"),
